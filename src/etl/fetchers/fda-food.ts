/**
 * openFDA Food Enforcement API Fetcher
 *
 * Endpoint: https://api.fda.gov/food/enforcement.json
 * Pagination: skip + limit (max 1000 per page)
 * Rate limit: 240 req/min with API key, 40/min without
 * Strategy: 250ms minimum between requests to stay well under limits
 */
...
export interface FdaFoodFetchOptions {
...
/**
 * Fetches all food enforcement records from the openFDA API.
 ...
 */
export async function fetchFdaFoodRecords(
  options: FdaFoodFetchOptions
  ...
  while (skip < totalAvailable) {
    ...
    const response = await withRetry(
      async () => {
        ...
        // Handle "no results" as a valid empty response
        if (res.status === 404) {
    ...
    if (response.error) {
      // "No matches found" is not a real error — just means no more records
